...
def replace_layer_names(psd, target, replacement):
    def replace_name(layer):
        if layer.name and target in layer.name:
            layer.name = layer.name.replace(target, replacement)

    traverse_layers(psd, replace_name)